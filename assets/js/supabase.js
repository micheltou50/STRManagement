...
export async function loadPropertyFromCloud() {
  try {
    const user = await getCurrentSupabaseUser();
    if (!user) {
...
export function _toLocalPatch(row, existing) {
  existing = existing || {};
...
// seedLocalConfigFromCloud + savePropertyToCloud moved to supabase-properties.js.
// Re-exported here; both imported back (the barrel calls them: saveHostConfigToCloud
// + hydration/init). loadPropertyFromCloud / _toLocalPatch / getCloudPropertyId stay
// here as the property-resolution core every slice depends on.
import { seedLocalConfigFromCloud, savePropertyToCloud } from './supabase-properties.js';
export { seedLocalConfigFromCloud, savePropertyToCloud } from './supabase-properties.js';
