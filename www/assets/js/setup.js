...
import { savePropertyToCloud } from './supabase.js';
/* ═══════════════════════════════════════════════════════════════════════════
...
function _setupBuildOverlay(editMode, createMode, onDone) {
  ...
  const doSubmit = (e) => {
    ...
    if (createMode) {
      ...
      // Full navigation (not location.reload) — reload is unreliable on iOS standalone PWA.
      setTimeout(() => {
  ...
  return {
    ...
    integrations: {
      vapidPublicKey:  _existingInteg.vapidPublicKey
                         || (DEFAULT_PROPERTY_CONFIG.integrations && DEFAULT_PROPERTY_CONFIG.integrations.vapidPublicKey) || '',
...
function _setupBlankConfig() {
  const d = DEFAULT_PROPERTY_CONFIG || {};
  return {
    propertyId: '',
    name: '',
    suburb: '',
