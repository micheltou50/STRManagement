...
- `Netlify/functions/` — serverless endpoints (cleaner actions, push, email reminders, Gmail/Outlook scanning, channel manager sync)
- `Netlify/functions/utils/` — shared helpers (auth, push)
- `index.html` — the entire app shell

## Key architectural decisions

### Bookings are soft-deleted
`deleteBooking()` in JS calls `deleteBookingFromCloud()` which issues a `.delete()`, but a Supabase trigger/RLS policy intercepts it and sets `status = 'cancelled'` instead of removing the row. The booking persists in the database. This applies both to manual deletes and platform cancellations (via channel manager / Gmail / Outlook scan). Cancelled bookings are always queryable.
...
- `bookings` — guest bookings (status: confirmed/cancelled)
